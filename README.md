...
| 1 | Win for player 1 | The first player achieved a line of the required length. | <ul><li>Row</li><li>Column</li><li>Diagonal</li></ul> | :white_check_mark: | <ul><li>check_row_win</li><li>check_col_win</li><li>check_diag_win</li><li>check_consecutive</li></ul> |
| 2 | Win for player 2 | The second player achieved a line of the required length. | see player1 win | :white_check_mark: | see player 1 win |
...
- Check if player has a potential win by;
    * checking if count of uninterupted moves by player meets win value by row / col / diagonal
- Check that potential win was last move of game (not illegal continue)
- Check if non-winning games are a draw (no moves available) or incomplete (possible moves remain)
    Note: the 'incomplete' check does not validate whether a win is achievable from the remaining moves, only that game not won and moves remain available
...
- Defining approach to assess outcome (turn by turn vs final status) and specifically diagonal search in 2 directions.


## Self Identified Improvements:
- Refactoring - given time there are opportunities to eliminate instances of similar checks (e.g. row / col / diag win by player A & B)
- Testing - was performed during the development with a total of c. 30 test files (9 from challenge plus 20+ additional). More thorough tests would have been achieved through unit testing and may identify possible scenarios not reflected in design / test examples.
